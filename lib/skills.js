...
function make(deps) {
  const {
    ...
    CUSTOM_SKILLS_ROOT, HUB_INSTALLED_ROOT,
...
禁止输出：解释、XML 标签、JSON。`,
  ...
  // skill 查找根目录优先级：custom > hub > redfox（用户的同名 skill 覆盖社区）
  function _skillRoots() {
    return [
      { root: CUSTOM_SKILLS_ROOT, source: 'custom' },
      { root: HUB_INSTALLED_ROOT, source: 'hub' },
      { root: SKILLS_ROOT, source: 'redfox' },
    ].filter(r => r.root);
  }

  function getSkill(slug) {
    if (!/^[a-z0-9][a-z0-9-]*$/i.test(slug)) return null;
    for (const { root, source } of _skillRoots()) {
      const skillPath = path.join(root, slug, 'SKILL.md');
      if (!skillPath.startsWith(`${root}${path.sep}`) || !fs.existsSync(skillPath)) continue;
      const skill = parseSkillFile(skillPath, root);
      return {
        ...skill,
        source,
        absPath: skillPath,
        isNew: source === 'redfox' && skillUpdateState().newSlugs.has(skill.slug),
      };
    }
    return null;
  }

  // 合并三目录（用于 Agent / 命令补全，去重：同名按 custom > hub > redfox 取）
  function listAllAgentSkills() {
    const map = new Map();
    for (const { root, source } of _skillRoots()) {
      if (!fs.existsSync(root)) continue;
      for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
        if (!entry.isDirectory()) continue;
        if (map.has(entry.name)) continue;  // 高优先级已存在则跳过
        const skillPath = path.join(root, entry.name, 'SKILL.md');
        if (!fs.existsSync(skillPath)) continue;
        const skill = parseSkillFile(skillPath, root);
        map.set(entry.name, { ...skill, source });
      }
    }
    return [...map.values()];
  }
...
`, archivePath, extractRoot], {
  ...
  function ensureCustomRoot() { return ensureDir(CUSTOM_SKILLS_ROOT); }
  function ensureHubRoot() { return ensureDir(HUB_INSTALLED_ROOT); }

  // 列某根目录下的 skill（共用）
  function listSkillsUnder(root, sourceLabel) {
    ...
    return fs.readdirSync(root, { withFileTypes: true })
      ...
      .map(e => {
        ...
        if (!fs.existsSync(skillPath)) return null;
        const skill = parseSkillFile(skillPath, root);
        return { ...skill, source: sourceLabel };
      })
      .filter(Boolean);
  }

  function listCustomSkills() {
    return listSkillsUnder(CUSTOM_SKILLS_ROOT, 'custom');
  }
  function listHubInstalled() {
    return listSkillsUnder(HUB_INSTALLED_ROOT, 'hub');
  }
  ...
  async function installHubSkill(slug, opts = {}) {
    ...
    ensureHubRoot();
    const targetDir = opts.asSlug ? path.join(HUB_INSTALLED_ROOT, opts.asSlug) : path.join(HUB_INSTALLED_ROOT, slug);
    if (fs.existsSync(targetDir) && !opts.force) throw new Error(`已安装 "${slug}"，强制覆盖传 force:true`);
  ...
  function uninstallHubSkill(slug) {
    const dir = path.join(HUB_INSTALLED_ROOT, slug);
    if (!dir.startsWith(`${HUB_INSTALLED_ROOT}${path.sep}`) || !fs.existsSync(dir)) {
      throw new Error(`"${slug}" 未安装`);
    }
    fs.rmSync(dir, { recursive: true, force: true });
