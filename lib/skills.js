...
function make(deps) {
  ...
  async function classifySkill(skill, retries = 3) {
    ...
    // 第 1 层：slug 规则确定性分类（不调 LLM）
    const slugCategory = classifyBySlug(skill.slug);
    if (slugCategory) {
      const now = Date.now();
      db.prepare(`
      ...
      `).run(skill.slug, slugCategory, skill.category, now, signature);
      return slugCategory;
    }

    // 第 2 层：LLM 兜底，带 few-shot 锚定
    ...
    const messages = [
      {
        role: 'system',
        content: `你是一个 Skill 分类器。根据 skill 的 slug / 标题 / 描述，把它归入以下七类之一：
...
严格只输出：<slug>:<类别>（无空格、无解释、无 XML 标签）。`,
      },
      {
        role: 'user',
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (LLM_SKILL_CATEGORIES.includes(category)) {
          // 冲突仲裁：若 slug 规则其实匹配过但被 LLM 推翻，slug 规则赢
          // （此处 slugCategory 为 null 才进 LLM 分支，所以无冲突；保留日志位以便日后加规则）
          const now = Date.now();
          db.prepare(`
            INSERT INTO skill_classifications (slug, llm_category, original_category, analyzed_at, skill_signature)
            VALUES (?, ?, ?, ?, ?)
    ...
    return existing?.llm_category || skill.category || '综合';
  }
