...
db.exec(`
  ...
  -- 单篇作品指标的时间序列：每次同步/插件上报追加一个点，用于增长曲线与同期增速评价
  CREATE TABLE IF NOT EXISTS work_stats_history (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    account_id TEXT NOT NULL,
    plat TEXT NOT NULL,
    work_id TEXT NOT NULL,
  ...
  -- 小红书 MCP 接入（xpzouying/xiaohongshu-mcp）
  -- base_url: HTTP API 入口（如 http://localhost:18060）
  -- mcp_url:  MCP 协议入口（如 http://localhost:18060/mcp），给 Agent CLI 用
  -- login_status: unknown / logged_in / logged_out（health check 后更新）
  CREATE TABLE IF NOT EXISTS xhs_mcp_config (
    id INTEGER PRIMARY KEY CHECK (id = 1),
    base_url TEXT NOT NULL,
    mcp_url TEXT,
    enabled INTEGER NOT NULL DEFAULT 1,
...
db.exec('CREATE INDEX IF NOT EXISTS idx_weekly_actions_week ON weekly_actions(week)');
db.exec('CREATE INDEX IF NOT EXISTS idx_work_stats ON work_stats_history(account_id, work_id, captured_at)');
db.exec('CREATE INDEX IF NOT EXISTS idx_article_analysis_account ON article_analysis(account_id, updated_at DESC)');
db.exec(`
