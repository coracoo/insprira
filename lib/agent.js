...
function make(deps) {
  ...
  async function executeLocalSpawnAgent(agentId, prompt, mode, outputFile, sessionId) {
    if (agentId === 'codex') {
    ...
    if (agentId === 'claude') {
      ...
      // 不加 --no-session-persistence：让 claude 把 session 写到
      // ~/.claude/projects/-app/（容器内 HOME 重定向后跟 data 卷走）
      // resume 模式：传 sessionId 时用 --resume <id>，接续上次对话
      const args = ['-p'];
      if (sessionId) {
        args.push('--resume', sessionId);
      }
      args.push('--permission-mode', mode === 'workspace' ? 'acceptEdits' : 'plan');
      args.push(prompt);
      const { stdout } = await runProcess(executable, args, { cwd: rootDir, timeout: 180000, maxBuffer: 5 * 1024 * 1024 });
      return stdout.trim();
    }
    if (agentId === 'kimi') {
  ...
  async function executeAgent(agentId, prompt, mode, outputFile, sessionId) {
    return USE_SBX
      ? executeSbxAgent(agentId, prompt, mode, outputFile)
      : executeLocalSpawnAgent(agentId, prompt, mode, outputFile, sessionId);
  }

  // —— session-id 提取（resume 用）——
  // 调用前快照、调用后比对，找出本次新生成的 session 文件
  const SESSION_DIRS = {
    claude: () => {
      const dir = path.join(os.homedir(), '.claude', 'projects', '-app');
      return fs.existsSync(dir) ? dir : null;
    },
    kimi: () => {
      const dir = path.join(os.homedir(), '.kimi-code', 'sessions');
      return fs.existsSync(dir) ? dir : null;
    },
  };

  function snapshotLatestSession(agentId) {
    const dir = SESSION_DIRS[agentId]?.();
    if (!dir) return null;
    try {
      const entries = fs.readdirSync(dir, { withFileTypes: true })
        .filter(e => e.isFile() && /\.jsonl$/.test(e.name))
        .map(e => {
  ...
  function pickLatestSessionId(agentId, before) {
    const dir = SESSION_DIRS[agentId]?.();
    if (!dir) return null;
    try {
      const entries = fs.readdirSync(dir, { withFileTypes: true })
      ...
      if (!before || latest.name !== before.name || latest.mtime !== before.mtime) {
  ...
  async function runLocalAgent(body) {
    ...
    if (skill) {
      const skillMdPath = skill.absPath || path.join(SKILLS_ROOT, skill.slug, 'SKILL.md');
      // 统一内联 SKILL.md 内容到 prompt：避免 CLI sandbox 限制读 /app 外路径
      // （custom/hub skill 在 /data/skills/，plan 模式可能拒读）
      if (fs.existsSync(skillMdPath)) {
        const skillContent = fs.readFileSync(skillMdPath, 'utf8');
        skillInstruction = `用户选择了本地 Skill：${skill.title}（来源：${skill.source || 'redfox'}）。SKILL.md 内容如下，按其中工作流执行：\n\n---\n${skillContent}\n---`;
      } else {
        skillInstruction = `用户选择了本地 Skill：${skill.title}（来源：${skill.source || 'redfox'}），但 SKILL.md 不存在（路径 ${skillMdPath}），按用户口头描述执行。`;
      }
    } else {
    ...
    try {
      sessionBefore = snapshotLatestSession(agentId);
      const answer = await executeAgent(agentId, prompt, mode, outputFile, body.sessionId);
      if (!answer) throw new Error(`${selectedAgent.name} 未返回内容`);
      const sessionId = pickLatestSessionId(agentId, sessionBefore);
      return { answer, agent: agentId, agentName: selectedAgent.name, skill: skill?.slug || null, mode, sessionId, resumed: Boolean(body.sessionId) };
    } catch (error) {
