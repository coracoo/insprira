...
module.exports = {
  ...
  getCliStatus() {
    ...
    return {
      ...
      // 宿主机路径（容器内 /app/data → 宿主 ./data）
      binPath: CLI_BIN_PATH.replace(/^\/app\/data\//, './data/'),
      binDir: CLI_BIN_DIR.replace(/^\/app\/data\//, './data/'),
      // 启动命令（宿主机执行；COOKIES_PATH 是环境变量不是 flag）
      startCommand: `COOKIES_PATH=./data/xhs-mcp/data/cookies.json ./data/xhs-mcp/bin/xiaohongshu-mcp`,
