...
//
// 超时说明：扫码登录（qrcode）需要 Chrome 打开小红书页面，耗时可能较长；
// 其他 API 是纯 HTTP 调用，20s 足够。

const DEFAULT_TIMEOUT = 20000;

async function request(baseUrl, path, options = {}) {
  const url = `${String(baseUrl).replace(/\/$/, '')}${path}`;
  // 仅允许 http/https，防 file:/gopher: 等协议走私（收敛 SSRF 面）
  // 注意：baseUrl 设计上就是内网地址（docker compose 内网 / localhost 本机），不能封禁内网网段
  let parsed;
  try { parsed = new URL(url); } catch { throw new Error(`xhs-mcp: baseUrl 非法（${baseUrl}）`); }
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error(`xhs-mcp: 仅支持 http/https 协议（当前 ${parsed.protocol}）`);
  }
