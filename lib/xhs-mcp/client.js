...
async function request(baseUrl, path, options = {}) {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error(`xhs-mcp: disallowed URL scheme "${parsed.protocol}"`);
  }
  const h = parsed.hostname.toLowerCase().replace(/^\[|\]$/g, '');
  if (/^(localhost$|127\.|10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.|169\.254\.)/.test(h) || h === '::1') {
    throw new Error('xhs-mcp: requests to internal addresses are not allowed');
  }
